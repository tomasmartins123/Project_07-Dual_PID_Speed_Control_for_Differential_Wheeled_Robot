// ============================================================================
// PROJETO CRIA 7 - Ensaio Automático Ki com Filtro Passa-Baixo (EMA)
// ============================================================================
...
// --- PARÂMETRO FIXO DE KP ---
const float Kp_fixo = 2.0; // Ganho Proporcional fixo durante o ensaio

// --- TESTE AUTOMÁTICO DE KI ---
float valoresKi[5] = {0.50, 1, 1.25, 1.5, 2.00};
int subEnsaio = 0;
...
void setup() {
  ...
  Serial.println("Tempo_ms,Ensaio,Ki,Setpoint,RPM_Medido,PWM_Out");
...
void loop() {
  ...
  if (agora - ultimoTempoAmostragem >= INTERVALO_AMOSTRAGEM) {
    ...
    if (tempoNoSubEnsaio >= 12000) { // 10s de Teste + 2s de Pausa concluídos
      ...
      // Se já testou os 5 valores de Ki, encerra o ensaio
      if (subEnsaio >= 5) {
        testeConcluido = true;
        analogWrite(PWM1, 0);
    ...
    float Ki_atual = valoresKi[subEnsaio];

    // 6. Lógica de Controlo vs Pausa
    if (tempoNoSubEnsaio >= 10000) {
      // --- FASE DE PAUSA (Últimos 2 segundos) ---
      // Força motor parado, limpa o filtro e reseta o acumulador integral para o próximo Ki
      pwmEsquerda = 0;
      rpmFiltrado = 0.0;
      acumuladorErro = 0.0;
    } else {
      // --- FASE DE TESTE (Primeiros 5 segundos) ---
      // Controlador PI Ativo
      float erroEsq = rpmDesejado - rpmFiltrado;

      // Acumula o erro no tempo (Termo Integral)
      acumuladorErro += erroEsq * dt;

      // Anti-Windup: Limita o acumulador
      acumuladorErro = constrain(acumuladorErro, -100.0, 100.0);

      // Sinal de controlo PI
      float sinalEsq = (Kp_fixo * erroEsq) + (Ki_atual * acumuladorErro);
    ...
    Serial.print(Ki_atual, 2); Serial.print(",");
